...
def get_config(config_file: str) -> edict:
    ...
    try:
        ...
        config_base = importlib.import_module("edgeface_lightning.configs.base")
        # copy()는 일반 dict를 반환하므로 edict()로 감싸야 함
        cfg = edict(config_base.config.copy())

        # Specific config 로드
        config_specific = importlib.import_module(f"edgeface_lightning.configs.{temp_module_name}")
