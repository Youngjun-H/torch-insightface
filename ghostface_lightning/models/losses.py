...
class ArcFaceLoss(nn.Module):
    ...
    def __init__(
        ...
        # Threshold for easy margin (like original: np.cos((np.pi - margin2) / margin1))
        # Note: Even when margin1=1.0, we still calculate threshold this way
    ...
    def forward(self, logits, labels):
        ...
        # Apply margin
        # For numerical stability with bfloat16, perform operations in float32
        target_logits_f32 = target_logits.float()

        # Calculate theta (like original: theta = tf.cos(tf.acos(y_pred_vals) + self.margin2))
        if self.margin1 == 1.0 and self.margin2 == 0.0 and self.margin3 == 0.0:
            theta = target_logits_f32
        elif self.margin1 == 1.0 and self.margin3 == 0.0:
            # Standard ArcFace: cos(theta + m)
            eps = 1e-7
            target_logits_clamped = target_logits_f32.clamp(-1 + eps, 1 - eps)
        ...
        # Apply easy margin: theta_valid = tf.where(y_pred_vals > self.threshold, theta, self.theta_min - theta)
        # (like original line 91: theta_valid = tf.where(y_pred_vals > self.threshold, theta, self.theta_min - theta))
        if self.easy_margin:
            theta_valid = torch.where(
                target_logits_f32 > self.threshold, theta, self.theta_min - theta
            )
        else:
            theta_valid = theta

        # Ensure theta_valid has the same dtype as logits (for bfloat16 mixed precision)
        theta_valid = theta_valid.to(dtype=logits.dtype)

        # Update logits (like original: tf.tensor_scatter_nd_update(norm_logits, pick_cond, theta_valid))
        arcface_logits = logits.clone()
        arcface_logits[target_mask] = theta_valid

        # Scale logits (like original: arcface_logits = tf.tensor_scatter_nd_update(...) * self.scale)
        arcface_logits = arcface_logits * self.scale

        # Clamp scaled logits to prevent overflow in cross_entropy (for numerical stability)
        # This is safe because logits are already normalized cosine similarities
        arcface_logits = arcface_logits.clamp(-100.0, 100.0)

        # Compute cross entropy loss
        loss = F.cross_entropy(
            arcface_logits, labels, label_smoothing=self.label_smoothing
        )

        # Check for NaN and replace with a large finite value to prevent training crash
        # Only replace if actually NaN/Inf (not for normal values)
        if torch.isnan(loss) or torch.isinf(loss):
            loss = torch.tensor(
                100.0, device=loss.device, dtype=loss.dtype, requires_grad=True
            )
...
class CosFaceLoss(nn.Module):
    ...
    def forward(self, logits, labels):
        ...
        # Scale logits (like original implementation)
        cosface_logits = cosface_logits * self.scale

        # Clamp scaled logits to prevent overflow in cross_entropy (for numerical stability)
        cosface_logits = cosface_logits.clamp(-100.0, 100.0)

        # Compute cross entropy loss
        loss = F.cross_entropy(
            cosface_logits, labels, label_smoothing=self.label_smoothing
        )
...
class AdaFaceLoss(nn.Module):
    ...
    def forward(self, logits_with_norm, labels):
        ...
        self.batch_mean = (
            self.mean_std_alpha * norm_mean
            + (1 - self.mean_std_alpha) * self.batch_mean
        )
        self.batch_std = (
            self.mean_std_alpha * norm_std + (1 - self.mean_std_alpha) * self.batch_std
        )

        # Compute adaptive margin
        margin_scaler = (feature_norm - self.batch_mean) / (
            self.batch_std + self.epsilon
        )
        ...
        # For numerical stability with bfloat16, perform operations in float32
        target_logits_f32 = target_logits.float()
        target_angles = target_logits_f32.clamp(-1 + eps, 1 - eps).acos()
        target_margins = scaled_margin[target_mask].float()

        # Apply margin: cos(angle - margin) - (margin + scaled_margin)
        target_angles_with_margin = (target_angles - target_margins).clamp(
            eps, cos_max_eps
        )
        target_logits_with_margin = target_angles_with_margin.cos() - (
            self.margin + target_margins
        )
        # Ensure dtype matches logits (for bfloat16 mixed precision)
        target_logits_with_margin = target_logits_with_margin.to(dtype=logits.dtype)
        adaface_logits[target_mask] = target_logits_with_margin

        # Scale logits (like original implementation)
        adaface_logits = adaface_logits * self.scale

        # Clamp scaled logits to prevent overflow in cross_entropy (for numerical stability)
        adaface_logits = adaface_logits.clamp(-100.0, 100.0)

        # Compute cross entropy loss
        loss = F.cross_entropy(
            adaface_logits, labels, label_smoothing=self.label_smoothing
        )
...
class MagFaceLoss(nn.Module):
    ...
    def __init__(
        ...
        self.margin_scale = (max_margin - min_margin) / (
            max_feature_norm - min_feature_norm
        )
    ...
    def forward(self, logits_with_norm, labels):
        ...
        margin = (
            self.margin_scale * (feature_norm - self.min_feature_norm) + self.min_margin
        )
        ...
        if self.use_cosface_margin:
            ...
            # For numerical stability with bfloat16, perform operations in float32
            target_logits_f32 = target_logits.float()
            target_angles = target_logits_f32.clamp(
                -1 + self.epsilon, 1 - self.epsilon
            ).acos()
            margin_f32 = margin.float()
            margin_cos = margin_f32.cos()
            margin_sin = margin_f32.sin()

            target_logits_with_margin = (
                target_logits_f32 * margin_cos
                - (1 - target_logits_f32.pow(2)).clamp(min=0.0).sqrt() * margin_sin
            )
            # Ensure dtype matches logits (for bfloat16 mixed precision)
            target_logits_with_margin = target_logits_with_margin.to(dtype=logits.dtype)
            magface_logits[target_mask] = torch.minimum(
                target_logits_with_margin, target_logits
            )
        ...
        # Clamp scaled logits to prevent overflow in cross_entropy (for numerical stability)
        magface_logits = magface_logits.clamp(-100.0, 100.0)

        # Compute cross entropy loss
        arcface_loss = F.cross_entropy(
            magface_logits, labels, label_smoothing=self.label_smoothing
        )

        # Regularizer loss: g = 1/(u_a^2) * x_norm + 1/(x_norm)
        regularizer_loss = (
            self.regularizer_loss_scale * feature_norm + 1.0 / feature_norm
        )
...
class CombinedMarginLoss(nn.Module):
    ...
    def __init__(
        ...
        # Threshold calculation (like original: np.cos((np.pi - m2) / m1))
        # Note: Even when m1=1.0, we still calculate threshold this way
    ...
    def forward(self, logits, labels):
        ...
        if self.m1 == 1.0 and self.m2 == 0.0 and self.m3 == 0.0:
            ...
            if self.m2 > 0:
                ...
                theta = target_logits_f32 - self.m3
        else:
            # General case: cos(m1 * theta + m2) - m3
            eps = 1e-7
            target_logits_clamped = target_logits_f32.clamp(-1 + eps, 1 - eps)
        ...
        # Apply easy margin: theta_valid = tf.where(y_pred_vals > self.threshold, theta, self.theta_min - theta)
        # (like original line 91: theta_valid = tf.where(y_pred_vals > self.threshold, theta, self.theta_min - theta))
        if self.easy_margin and self.m2 > 0:
            theta_min = (-1 - self.m3) * 2
            theta_valid = torch.where(
                target_logits_f32 > self.threshold, theta, theta_min - theta
            )
        else:
            theta_valid = theta
        ...
        margin_logits[target_mask] = theta_valid

        # Scale logits (like original implementation)
        margin_logits = margin_logits * self.s

        # Clamp scaled logits to prevent overflow in cross_entropy (for numerical stability)
        margin_logits = margin_logits.clamp(-100.0, 100.0)

        # Compute cross entropy loss
        loss = F.cross_entropy(
            margin_logits, labels, label_smoothing=self.label_smoothing
        )
